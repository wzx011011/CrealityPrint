...
void main( void )
{
    vs_out.color   = palette[vertexFlag] * stateGain[state];
	...
	vs_out.gnormal = normalize(mat3(ggnmatrix) * vertexNormal);

    vec4 pos = vec4(vert, 1.0);
    vs_out.worldPosition = vec3(model_matrix * pos);

    vs_out.intensity = lightIntensity(normal, vs_out.worldPosition.xyz);

    vs_out.pos = projectionMatrix * modelViewMatrix * pos;
}
