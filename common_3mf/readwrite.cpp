...
const char* _BBS_3MF_VERSION1 = "bamboo_slicer:Version3mf"; // definition of the metadata name saved into .model file
const char* _BBS_3MF_VERSION = "BambuStudio:3mfVersion"; //compatible with prusa currently
...
const char* _BBS_VALID_OBJECT_TYPES[] =
{
    "model",
    "other"
};
...
namespace common_3mf
{
    ...
    bool bbs_is_valid_object_type(const std::string& type)
    {
        ...
        for (unsigned int i = 0; i < BBS_VALID_OBJECT_TYPES_COUNT; ++i) {
            if (::strcmp(type.c_str(), _BBS_VALID_OBJECT_TYPES[i]) == 0)
                return true;
        }
	...
	bool Read3MFImpl::read_all_3mf(Scene3MF& scene,bool use_escaping, ccglobal::Tracer* tracer)
	{
        this->m_errors.clear();
        m_objects_list.clear();
        m_use_escaping = use_escaping;
        if (!m_open_success)
            return false;
...
#else
            for (auto object_importer : m_object_importers) {
                bool result = object_importer->extract_object_model();
                object_load_result &= result;
            }
#endif
        ...
        for (mz_uint i = 0; i < num_entries; ++i) {
            if (mz_zip_reader_file_stat(&m_archive, i, &stat)) {
                ...
                else if (name == SLICE_INFO_CONFIG_FILE)
                {
                    _extract_slice_info_from_archive(m_archive, stat);
                }
    ...
    bool Read3MFImpl::_extract_xml_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat, XML_StartElementHandler start_handler, XML_EndElementHandler end_handler)
    {
        ...
        if(std::string(stat.m_filename) == "3D/_rels/3dmodel.model.rels" && m_use_escaping)
        {
            std::string rels_content = std::string((const char*)parser_buffer, stat.m_uncomp_size);
            std::regex targetRegex(R"(Target=\"([^\"]*)\")");
            ...
            while (iter != end) {
                ...
                auto pos = rels_content.find(content);
                if(pos != std::string::npos)
                {
                    rels_content = rels_content.replace(pos,content.size(),newTarget);
                }
                ++iter;
            }
            memcpy(parser_buffer, rels_content.c_str(), rels_content.size());
            datalen = rels_content.size();
    ...
    void Read3MFImpl::_extract_slice_info_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)
    {
        return;
    }
    void Read3MFImpl::_extract_model_settings_from_archive(mz_zip_archive& archive, const mz_zip_archive_file_stat& stat)
    {
        ...
        mz_file_write_func callback = [](void* pOpaque, mz_uint64 file_ofs, const void* pBuf, size_t n)->size_t {
            CallbackData* data = (CallbackData*)pOpaque;
            std::string rels_content = std::string((const char*)pBuf, n);
            if(data->importer.m_use_escaping)
            {
                std::regex targetRegex(R"(key=\"name\".*value=\"([^\"]*)\")");
                std::sregex_iterator iter(rels_content.begin(), rels_content.end(), targetRegex);
            ...
            if (!XML_Parse(data->parser, (const char*)rels_content.c_str(), (int)rels_content.size(), (file_ofs + n == data->stat.m_uncomp_size) ? 1 : 0)) {
                char error_buf[1024];
                ::snprintf(error_buf, 1024, "Error while parsing '%s' at line %d", data->stat.m_filename, (int)XML_GetCurrentLineNumber(data->parser));
                return false;
            }
    ...
    void Write3MFImpl::append_file(const std::string& file_name, const std::string& entry_name)
    {
        if (!m_open_success)
            return;

        mz_zip_writer_add_file(&m_archive, entry_name.c_str(), file_name.c_str(), NULL, 0, MZ_DEFAULT_LEVEL);
    }
    ...
    bool Write3MFImpl::_add_model_relations_file(const std::vector<Group3MF>& groups)
    {
        ...
        for (const Group3MF& group : groups)
        {
            ...
            stream << " <Relationship Target=\"/" << xml_escape(ppath) << "\" Id=\"rel-"<<index<<"\" Type=\"http://schemas.microsoft.com/3dmanufacturing/2013/01/3dmodel\"/>\n";
            index++;
        }
    ...
    bool Write3MFImpl::_add_scene_content(const Scene3MF& scene)
    {
...

        {
            ...
            {
                boost::algorithm::to_lower_copy(BBL_COPYRIGHT_TAG), 
                boost::algorithm::to_lower_copy(BBL_COPYRIGHT_NORMATIVE_TAG),
    ...
    bool Write3MFImpl::_add_model_setting_file(const Scene3MF& scene)
    {
        ...
        auto write_meta = [&stream](const std::string& k, const std::string& v) {
            stream << "    <" << METADATA_TAG << " key=\"" << k << "\" value=\"" << xml_escape(v) << "\"/>\n";
        };
