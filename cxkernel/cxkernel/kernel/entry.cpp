...
namespace cxkernel
{
    ...
    const QDateTime monthAgoDateTime = currentDateTime.addDays(-10);

    QFileInfoList fileList = dir.entryInfoList(QDir::Files);
    for (const QFileInfo &fileInfo : fileList) {
        if (fileInfo.lastModified() < monthAgoDateTime) {
...
#else
    	QCoreApplication::setApplicationName(QStringLiteral("E-Tech"));
#endif

		QString logDirectory = qtuser_core::getOrCreateAppDataLocation("Log");
		deleteOldFiles(logDirectory);
		auto func = [](const char* name)->std::string {
			QString  filename = "%1_%2.cxlog";
			filename= filename.arg(QDateTime::currentDateTime().toString("yyyy-MM-dd")).arg(QCoreApplication::applicationPid());
			return filename.toLocal8Bit().data();
		};
		auto max_size = 1048576 * 10;
		LOGINITX(logDirectory.toStdString() +"/"+ func("cxlog"),max_size);
	...
	int appMain(int argc, char* argv[], AppModuleCreateFunc func)
	{
		...
		app.installEventFilter(openEventFilter);
#ifndef Q_OS_WIN
		struct rlimit limit;
