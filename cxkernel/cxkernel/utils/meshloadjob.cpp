...
namespace cxkernel
{
	...
	void MeshLoadJob::work(qtuser_core::Progressor* progressor)
	{
		...
		if(info.suffix().toLower()=="3mf")
		{
			common_3mf::Read3MF *reader = new common_3mf::Read3MF(cxkernel::qString2String(m_fileName));
			if (reader->read_all_3mf(m_scene, false,&tracer))
			{
				delete reader;
			}else
			{
				reader = new common_3mf::Read3MF(cxkernel::qString2String(m_fileName));
				reader->read_all_3mf(m_scene, true,&tracer);
				delete reader;
			}
		}else
#endif
