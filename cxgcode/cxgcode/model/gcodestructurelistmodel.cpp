...
void GcodeStructureListModel::setOrcaTimeParts(std::vector<std::pair<int, float>> time_parts,int total_time)
{
    ...
    for (int i = 0; i < time_parts.size(); i++)
    {
        ...
        switch (static_cast<Orca::ExtrusionRole>(pair.first))
        {
        case Orca::ExtrusionRole::erPerimeter:
               inner_wall_time = static_cast<float>(pair.second);
               data_list.append({QColor{ QStringLiteral("#FFE64D") }, QStringLiteral("Inner Perimeter") , sec2str(inner_wall_time)        , inner_wall_time/total_time * 100.0     , static_cast<int>(Orca::ExtrusionRole::erPerimeter) , true  });
               break;
           case Orca::ExtrusionRole::erExternalPerimeter:
               outer_wall_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#FF7D38") }, QStringLiteral("Outer Perimeter") , sec2str(outer_wall_time)        , outer_wall_time/total_time * 100.0     , static_cast<int>(Orca::ExtrusionRole::erExternalPerimeter) , true  });
               break;
           case Orca::ExtrusionRole::erOverhangPerimeter:
               overhang_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#0000FF") }, QStringLiteral("Overhang Wall")            , sec2str(overhang_time)              , overhang_time/total_time * 100.0         , static_cast<int>(Orca::ExtrusionRole::erOverhangPerimeter) , true });
               break;
           case Orca::ExtrusionRole::erInternalInfill:
               internal_infill_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#B03029") }, QStringLiteral("Sparse infill")            , sec2str(internal_infill_time)              , internal_infill_time/total_time * 100.0         , static_cast<int>(Orca::ExtrusionRole::erInternalInfill) , true });
               break;
           case Orca::ExtrusionRole::erSolidInfill:
               solid_infill_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#9654CC") }, QStringLiteral("Internal solid infill")         , sec2str(solid_infill_time)           , solid_infill_time/ total_time * 100.0       , static_cast<int>(Orca::ExtrusionRole::erSolidInfill) , true });
               break;
           case Orca::ExtrusionRole::erTopSolidInfill:
               top_solid_infill_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#F04040") }, QStringLiteral("Top surface"), sec2str(top_solid_infill_time), top_solid_infill_time / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erTopSolidInfill), true  });
               break;
           case Orca::ExtrusionRole::erBottomSurface:
               bottom_solid_infill_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#665CC7") }, QStringLiteral("Bottom surface"), sec2str(bottom_solid_infill_time), bottom_solid_infill_time/ total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erBottomSurface), true });
               break;
           case Orca::ExtrusionRole::erIroning:
               ironing = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#FF8C69") }, QStringLiteral("Ironing"), sec2str(ironing), ironing / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erIroning), true });
               break;
          case Orca::ExtrusionRole::erBridgeInfill:
               bridge_infill = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#4D66A1") }, QStringLiteral("Bridge"), sec2str(bridge_infill), bridge_infill / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erBridgeInfill), true });
               break;
           case Orca::ExtrusionRole::erSkirt:
               skirt_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#00876E") }, QStringLiteral("Skirt"), sec2str(skirt_time), skirt_time / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erSkirt), true });
               
               break;
           case Orca::ExtrusionRole::erBrim:
               brim_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#003B6E") }, QStringLiteral("Brim"), sec2str(brim_time), brim_time / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erBrim), true });

               break;
           case Orca::ExtrusionRole::erGapFill:
               gapfill_time = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#FFFFFF") }, QStringLiteral("Gap Fill"), sec2str(brim_time), brim_time / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erGapFill), true });

               break;
           case Orca::ExtrusionRole::erSupportMaterial:
               support_material = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#00FF00") }, QStringLiteral("Support"), sec2str(support_material), support_material / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erSupportMaterial), true });
               break;
           case Orca::ExtrusionRole::erSupportMaterialInterface:
               support_material_interface = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#008000") }, QStringLiteral("Support interface"), sec2str(support_material_interface), support_material_interface / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erSupportMaterialInterface), true });
               break;
           case Orca::ExtrusionRole::erSupportTransition:
               support_transition = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#004000") }, QStringLiteral("Support transition"), sec2str(support_transition), support_transition / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erSupportTransition), true });
               break;
           case Orca::ExtrusionRole::erWipeTower:
               wipe_tower = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#B3E3AB") }, QStringLiteral("Prime tower"), sec2str(wipe_tower), wipe_tower / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erWipeTower), true });
               break;
           case Orca::ExtrusionRole::erCustom:
               custom = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#5ED194") }, QStringLiteral("Custom"), sec2str(custom), custom / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erCustom), true });
               break;
           case Orca::ExtrusionRole::erInternalBridgeInfill:
               internalBridge = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#4D80BA") }, QStringLiteral("Internal Bridge"), sec2str(internalBridge), internalBridge / total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::erInternalBridgeInfill), true });
               break;
           case Orca::ExtrusionRole::Noop:
               noop = static_cast<float>(pair.second);
               break;
           case Orca::ExtrusionRole::Retract:
               retract = static_cast<float>(pair.second);
               
               break;
           case Orca::ExtrusionRole::Unretract:
               unretract = static_cast<float>(pair.second);
               
               break;
           case Orca::ExtrusionRole::Seam:
               seam = static_cast<float>(pair.second);
               
               break;
           case Orca::ExtrusionRole::Tool_change:
               tool_change = static_cast<float>(pair.second);
               break;
           case Orca::ExtrusionRole::Color_change:
               color_change = static_cast<float>(pair.second);
               break;
           case Orca::ExtrusionRole::Pause_Print:
               pause_Print = static_cast<float>(pair.second);
               break;
           case Orca::ExtrusionRole::Custom_GCode:
               custom_gcode = static_cast<float>(pair.second);
               break;
           case Orca::ExtrusionRole::Travel:
               travel = static_cast<float>(pair.second);
               data_list.append({ QColor{ QStringLiteral("#38489B") }, QStringLiteral("Travel"), sec2str(travel), travel/ total_time * 100.0, static_cast<int>(Orca::ExtrusionRole::Travel), false });
               break;
           case Orca::ExtrusionRole::Wipe:
               wipe = static_cast<float>(pair.second);
               
               break;
           case Orca::ExtrusionRole::Extrude:
    ...
    data_list.append({ QColor{ QStringLiteral("#CD22D6") }, QStringLiteral("Retract"), 0, 0, static_cast<int>(Orca::ExtrusionRole::Retract), false });
    data_list.append({ QColor{ QStringLiteral("#49ADCF") }, QStringLiteral("Unretract"), 0, 0, static_cast<int>(Orca::ExtrusionRole::Unretract), false });
    data_list.append({ QColor{ QStringLiteral("#FFFF00") }, QStringLiteral("Wipe"), 0, 0, static_cast<int>(Orca::ExtrusionRole::Wipe), false });
    data_list.append({ QColor{ QStringLiteral("#FFFFFF") }, QStringLiteral("Seams"), 0, 0, static_cast<int>(Orca::ExtrusionRole::Seam), true });
