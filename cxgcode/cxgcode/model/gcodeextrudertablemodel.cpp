...
{
    auto get = [](std::vector<std::pair<int, double>>& data, int filament, bool isLength)
    {
        int count = isLength ? 0 : 1;
        for (auto& pair : data) {
            if (pair.first == filament) {
                if (count == 0)
                    return pair.second;
    ...
    for (int index = 0; index < model.size(); ++index)
    {
        ...
        if (isLength)
        {
            ...
            if (flush.size() > index)
                mc_flush->m_Length = m_IsSingleColor ? 0 : get(flush, mc_flush->m_MaterialIndex, true);
            if(filamentTower.size() > index)
                mc_filamentTower->m_Length = m_IsSingleColor ? 0 : get(filamentTower, mc_filamentTower->m_MaterialIndex, true);
            mc_total->m_Length = mc_model->m_Length + mc_flush->m_Length + mc_filamentTower->m_Length;
        }
        else {
            mc_model->m_Weight = model[index].second;
            if (flush.size() > index)
                mc_flush->m_Weight = m_IsSingleColor ? 0 : get(flush, mc_flush->m_MaterialIndex, false);
            if (filamentTower.size() > index)
                mc_filamentTower->m_Weight = m_IsSingleColor ? 0 : get(filamentTower, mc_filamentTower->m_MaterialIndex, false);
