...
namespace cxcloud {
  ...
  namespace {
    ...
    auto EnsureDirPathExists(const QString& path_string) -> bool {
      #ifdef Q_OS_LINUX
        auto path = filesystem::path{ path_string.toStdString() };
      #else
  ...
  auto HttpRequest::getHeader() const -> HttpHeader {
    ...
    header_ = {
      ...
      { QStringLiteral("__CXY_APP_ID_"), QStringLiteral("E-Tech_model") },
  ...
  auto HttpRequest::syncDownload() -> void {
    ...
    const auto response = [this, &session, &download_path] {
      #ifdef Q_OS_LINUX
        auto path = filesystem::path{ download_path.toStdString() };
      #else
