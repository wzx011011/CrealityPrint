...
namespace cxcloud {
  ...
  auto ProjectService::importProjectFromUrl(const QUrl& url) -> void
  {
    QUrlQuery query(url.query());
    QString file_3mf = QStandardPaths::writableLocation(QStandardPaths::DownloadLocation) + "/temp.3mf";
    if(query.hasQueryItem("name"))
    {
      file_3mf = QStandardPaths::writableLocation(QStandardPaths::DownloadLocation) + "/" + query.queryItemValue("name");
    }
    QElapsedTimer timer;
    timer.start();
    download_request_ = createRequest<DownloadRequest>(url.toString().toStdString().c_str(), file_3mf);
    download_request_->setSuccessedCallback([this, url,file_3mf,timer] {
        if (!open_file_handler_) {
          importProjectUrlFailed(url.toString());
          return;
        }
        if(QFileInfo(file_3mf).exists())
        {
          ...
          qWarning() << "importProjectFromUrl elapsed:" << elapsed;
          open_file_handler_(file_3mf);
        }
        else
        {
          importProjectUrlFailed(url.toString());
        }
        //importProjectUrlFailed(url);
      });

    download_request_->setFailedCallback([this, url] {
        importProjectFailed(url.toString());
      });
