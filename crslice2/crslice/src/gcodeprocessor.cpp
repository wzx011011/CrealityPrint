...
namespace crslice2
{
    ...
    void process_file(const std::string& file, GCodePath& gcodePath)
    {
        Slic3r::GCodeProcessor processor;
        processor.process_file(file.data());

        Slic3r::GCodeProcessorResult result;
        result = std::move(processor.extract_result());

        Slic3r::GUI::GCodeRenderGenerator renderGenerator;
        renderGenerator.init(Slic3r::ConfigOptionMode::comSimple, true);
        renderGenerator.load(result);
        gcodePath = renderGenerator.gcodePath();
