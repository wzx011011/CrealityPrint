...
namespace crslice2
{
	...
	void CacheSlice::volume_slices(int index, ccglobal::VisualDebugger* debugger)
	{
		...
		{
			trimesh::vec4 volume_color = indexColor(i);
			...
			for (int j = 0; j < layer_count; ++j)
			{
				...
				CovertParam param;
				param.out = volume_color;
				param.in = volume_color * 0.5f;
		...
		for (int i = 0; i < layer_count; ++i)
		{
			...
			for (int j = 0; j < region_count; ++j)
			{
				...
				for (const Slic3r::Surface& surf : sc.surfaces)
				{
					...
					CovertParam param;
					param.out = vparam.visual_absolute ? vparam.color : indexColor(j);
					if (vparam.as_type)
					{
						param.out = surfaceTypeColor(surf.surface_type);
					}
					param.width = surfaceTypeWidth(surf.surface_type);
					param.in = param.out * 0.5f;
					param.z = z;
