...
bool detect_creality_ams(const Slic3r::DynamicPrintConfig& config, ccglobal::Tracer* tracer)
{
	...
	if (_config)
	{
		...
		if (boost::starts_with(printer_model.c_str(), "K2 Plus") 
			...
			|| boost::starts_with(printer_model.c_str(), "K1")
			|| boost::starts_with(printer_model.c_str(), "Creality Hi"))
		{
			return true;
		}
	}
...
void orca_slice_impl_result(Slic3r::Print& print, Slic3r::Model& model, Slic3r::DynamicPrintConfig& config, const std::string& out_file, const std::string& temp_directory
	...
	, OrcaResult& result, ccglobal::Tracer* tracer, Slic3r::Vec3d plate_origin)
{
	SYSTEM_TICK("verify ->");
	const Slic3r::t_config_option_keys keys = config.def()->keys();
	for (const Slic3r::t_config_option_key& key : keys)
	...
	//it will crash if we don't catch any unhandled slice exception which raise from print.process
	catch (...)
	{
		exception = std::current_exception();
	}

	//throw any unhandled exception as crslice2::CrSliceException
	if (exception)
	{
		try
		{
		...
		{
			throw crslice2::CrSliceException(e.what(), 0);
		}
	}
...
Slic3r::SlicingParameters getSliceParam(crslice2::SettingsPtr settings,  Slic3r::ModelObject* currentObject)
{
	...
	return Slic3r::PrintObject::slicing_parameters(config, *currentObject, 0.f, Slic3r::Vec3d(1.0,1.0,1.0));
}
