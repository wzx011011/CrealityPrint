...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = true;
    {
        ...
        std::string bambuslicer_gcode_header1 = "; BambuStudio";

        //creality
        std::string creality_gcode_header = std::string("; generated by ");
        creality_gcode_header += SLIC3R_APP_NAME;

        std::string creality_gcode_header_60 = std::string("; generated by ");
        creality_gcode_header_60 += "CrealityPrint";
        
        std::string orcaslicer_gcode_header = std::string("; generated by ");
        orcaslicer_gcode_header += SLIC3R_APP_KEY;
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0 ||
                strncmp(bambuslicer_gcode_header1.c_str(), line_c, strlen(bambuslicer_gcode_header1.c_str())) == 0 ||
                strncmp(orcaslicer_gcode_header.c_str(), line_c, strlen(orcaslicer_gcode_header.c_str())) == 0 ||
                strncmp(creality_gcode_header.c_str(), line_c, strlen(creality_gcode_header.c_str())) == 0 ||
                strncmp(creality_gcode_header_60.c_str(), line_c, strlen(creality_gcode_header_60.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
