...
bool use_default_real_value = false;
...
// Only add a newline in case the current G-code does not end with a newline.
                                                  ...
                                                  double                             z) const
    {
        ...
        if (should_travel_to_tower) {
            ...
            gcode += gcodegen.retract(false, false, gcodegen.m_writer.get_lift_type());
        ...
        if (!is_creality_ams() && will_go_down) {
            gcode += gcodegen.writer().retract();
            gcode += gcodegen.writer().travel_to_z(z, "Travel down to the last wipe tower layer.");
            gcode += gcodegen.writer().unretract();
        }
...
//    }
    ...
    static void update_print_estimated_stats(GCodeProcessor& processor, const std::vector<Extruder>& extruders, PrintStatistics& print_statistics, const PrintConfig& config, PrintEstimatedStatistics& ps)
    {
        const GCodeProcessorResult& result = processor.get_result();
        ...
        ps.total_extruded_volume = total_extruded_volume;
        ps.total_used_filament = total_used_filament;
        ps.total_weight = total_weight;
...
//    DoExport::update_print_estimated_times_stats(m_processor, print->m_print_statistics);
    DoExport::update_print_estimated_stats(m_processor, m_writer.extruders(), print->m_print_statistics, print->config(), m_processor.m_result.print_statistics);
    if (result != nullptr) {
...
std::string GCode::set_extruder(unsigned int extruder_id, double print_z, bool by_object)
{
    ...
    this->placeholder_parser().set("z_after_toolchange", new ConfigOptionFloat(nozzle_pos(2)));
