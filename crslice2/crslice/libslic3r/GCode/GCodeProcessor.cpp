...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    ...
    { EProducer::OrcaSlicer, "generated by E-Tech_Print" },
    { EProducer::OrcaSlicer, "generated by E-TechPrint" },
...
bool GCodeProcessor::process_orca_tags(const std::string_view comment)
{
    if (boost::starts_with(comment, " estimated printing time (normal mode) = ")) {
        auto data = comment.substr(strlen(" estimated printing time (normal mode) = "));
...
void GCodeProcessor::begin_process_creality_image(const std::string& comment)
{
    ...
    image_data_cache.push_back(std::string(""));
...
bool GCodeProcessor::process_creality_image(const std::string& comment)
{
    if (m_reading_image)
    {
        if (comment.find(" end") != std::string_view::npos) {
            if (boost::starts_with(comment, " thumbnail end") ||
                boost::starts_with(comment, " png end") ||
                boost::starts_with(comment, " jpg end") ||
                boost::starts_with(comment, " bmp end") ||
                boost::starts_with(comment, " thumbnail_JPG end") ||
                boost::starts_with(comment, " thumbnail_QOI end")) {
        ...
        image_data_cache.back() += _comment;
...
void GCodeProcessor::end_process_creality_image()
{
    if (!image_data_cache.empty()) {
        ...
        for (auto& str : image_data_cache)
        {
            std::vector<std::string> inPrevData;
            inPrevData.push_back(str);
            images.push_back(base2image(inPrevData[0]));
        }

        {
            int max_index = image_data_cache.size() - 1;
            if (max_index >= 0)
                m_result.image_data.push_back(std::pair<int, std::vector<unsigned char>>(images[max_index].size(), images[max_index]));
        }
...
bool GCodeProcessor::process_creality_tags(const std::string_view comment)
{
    ...
    if (comment.find(" begin") != std::string_view::npos) {
        if (boost::starts_with(comment, " thumbnail begin") ||
            boost::starts_with(comment, " png begin") ||
            boost::starts_with(comment, " jpg begin") ||
            boost::starts_with(comment, " bmp begin") ||
            boost::starts_with(comment, " thumbnail_JPG begin") ||
            boost::starts_with(comment, " thumbnail_QOI begin")) {
    ...
    if (boost::starts_with(comment, " default_filament_colour = ")) {
        ...
        boost::split(m_result.creality_extruder_colors_with_unused, data.data(), boost::algorithm::is_any_of(";"));
        return true;
    }
    ...
    if (boost::starts_with(comment, " default_filament_type = ")) {
        auto data = comment.substr(strlen(" default_filament_type = "));
        std::istringstream iss(data.data());
        std::string item;
        boost::split(m_result.creality_fliament_types, data.data(), boost::algorithm::is_any_of(";"));
        return true;
    }
...
{
    if (process_orca_tags(comment))
        return true;

    if (boost::starts_with(comment, " model printing time: "))
    {
        auto data = comment.substr(strlen(" model printing time: "));
        std::istringstream iss(data.data());
        //std::string token;
        //while (iss >> token) {
                if (data.find("total estimated time:") != std::string::npos) {
                    std::size_t pos = data.find(":");
                        if (pos != std::string::npos) {
                            std::string result = data.data();
                            result = result.substr(pos + 1);
                            this->m_result.print_statistics.total_estimated_time = timeStringToInt(result);
                        }
                }
            //}
    }
...
void GCodeProcessor::store_move_vertex(EMoveType type, EMovePathType path_type)
{
    ...
    Vec3f position(m_end_position[X] + m_x_offset,
                m_end_position[Y] + m_y_offset,
                m_processing_start_custom_gcode ? m_first_layer_height : m_end_position[Z] - m_z_offset);

    position += m_extruder_offsets[m_extruder_id];

    Vec3f arcPosition(m_arc_center(0, 0) + m_x_offset, 
                                 m_arc_center(1, 0) + m_y_offset, 
                                 m_arc_center(2, 0) - m_z_offset);

    arcPosition += m_extruder_offsets[m_extruder_id];
