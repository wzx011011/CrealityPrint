...
CusBackground {
    ...
    function startPlayVideo() {
        __LANload.item.startPlayVideo();
    }
    function stopPlayVideo() {
        __LANload.item.stopPlayVideo();
    }
    ...
    Component.onCompleted: function() {
        ...
        if(kernel_ui.checkFirstStart())
        {
            editorContent.requestFirstConfig();
        }
        else
        {
            initOtherUi();
    ...
    QuickScene3DWrapper {
        ...
        function requestFirstConfig() {
            ...
            receiver.onSigClose = function(){
                initOtherUi();
            };
            idAllMenuDialog.requestMenuDialog(receiver,"idStartFirstConfig");
        }
            ...
            CSliceButton {
                ...
                onExportButtonClicked:{
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        fileDialog.title = qsTranslate("creative_kernel::Save3MFCommand", "Save 3MF")
                        fileDialog.nameFilters = [ "Gcode files (*.gcode)", "3MF files (*.3mf)" ]
                        fileDialog.funcPtr = saveOne
                        fileDialog.open()
                        break;
                    case 3:
                        //                        fileDialog.title = qsTranslate("creative_kernel::Save3MFCommand", "Save 3MF")
                        //                        fileDialog.nameFilters = ["3MF files (*.3mf)"]
                        //                         fileDialog.funcPtr = saveAll
                        //                        fileDialog.open()
                ...
                function saveOne(filePath){
                    console.warn("saveOne = ", filePath)
                    let fileUrl = filePath.toString()
                    if(fileDialog.selectedNameFilter.index==0){
                        kernel_slice_flow.saveGCodeLocal(filePath);
                    }else if(fileDialog.selectedNameFilter.index==1){
                        project_kernel.exportSinglePlate(filePath)
                    }
                }

                function saveAll(filePath){
                    project_kernel.exportAllPlate()
                }
            }
    ...
    DropArea {
        ...
        onDropped: {
            if (drop.hasUrls)
            {
                ...
                {
                    let filepath = kernel_kernel.convertUrlToLocalPath(fileUrl.toString())
                            ...
                            fileExtension.toLowerCase() === "3mf")
                    {
                        if(project_kernel.isProjectOpened())
                        {
                            ...
                            receiver.getSupportGeometry = function() {
                                return fileExtension.toLowerCase() === "3mf";
                            };
        ...
        onSigConfirmBtnClicked: {
            if (uploadDlgType!==4) {
                ...
                if(uploadDlgType===0)
                {
                    ...
                    __LANload.item.setCurGcodeFileName(__LANload.curGcodeFileName);
                    __LANload.item.setCurGcodeFilePath(gcodeFilePath);
                }
                else
                {
                    __LANload.curGcodeFileName = gcodeFileName+".3mf";
                    __LANload.item.setCurGcodeFileName(__LANload.curGcodeFileName);
                    __LANload.item.setCurGcodeFilePath(gcodeFilePath);
                }
                ...
                kernel_kernel.setKernelPhase(2);
                __LANload.item.setRealEntry(true);
            } else {
                curWindowState = UploadGcodeDlg.WindowState.State_Progress;
                cxkernel_cxcloud.sliceService.uploadSlice(curGcodeFileName);
            }
        }
        onSigDetailBtnClicked:function(printerID, printerName, printerType) {
            //visible = false;
            kernel_kernel.setKernelPhase(2);
            __LANload.item.openDetail(printerID, printerName, printerType)
    ...
    FileDialog {
        ...
        onAccepted: {
            console.warn("fileDialog.file = ", fileDialog.file)
            callFunctionPointer(funcPtr, fileDialog.file)
            project_kernel.projectTypeIndex = selectedNameFilter.index
        }

        function callFunctionPointer(func, arg) {
            if (typeof func === "function") {
                console.warn("callFunctionPointer")
                func(arg);
            }
        }
        ...
        onVisibleChanged: {
            if(visible){
                ...
                currentFile = kernel_slice_flow.getExportUrl()
            }
        }
    }
