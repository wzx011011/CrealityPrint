...
Rectangle {
        ...
        onPositionChanged: {
            if (Qt.platform.os === "linux")
            {
                ...
                else
                {
                    ...
                    if (frameLessView.width >= Screen.desktopAvailableWidth)
                    {
                        console.log(Qt.formatDateTime(new Date(), "yyyy-MM-dd hh:mm:ss.zzz"), " CWinMenu x=",pos.x, ",y=", pos.y);
                        console.log(Qt.formatDateTime(new Date(), "yyyy-MM-dd hh:mm:ss.zzz"), " 1 width=",frameLessView.width,",screenWidth=",Screen.desktopAvailableWidth)
                        frameLessView.showMaximized();
                    }
                    else
                    {
                        frameLessView.setX(frameLessView.x + pos.x);
                        frameLessView.setY(frameLessView.y + pos.y);
    ...
    Row {
        ...
        Loader
        {
            active : Qt.platform.os === "windows" || Qt.platform.os === "linux"
