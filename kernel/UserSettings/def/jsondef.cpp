...
namespace us
{
	...
	void JsonDef::parse(const QString& fileName, QMap<QString, SettingGroupDef*>& defs, QObject* parent)
	{
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text))
        {
            ...
            std::function<void(const rapidjson::Value& value, SettingItemDef* itemDef, QString classType)> fillItemCall;
            fillItemCall = [&fillItemCall](const rapidjson::Value& value, SettingItemDef* itemDef, QString classType) {
                if (!value.IsObject()) return;

                itemDef->classType = classType;
                itemDef->order = orderCnt++;
                ...
                if (value.HasMember("parameter_level")) itemDef->paramLevel = (value["parameter_level"].GetString());
                ...
                if (value.HasMember("children"))
                {
                        ...
                        child != children.MemberEnd(); child++)
                    {
                        ...
                        fillItemCall(childValue, settingItemDef, classType);
                    }
                }
            ...
            auto fillGroup = [&fillItemCall](const rapidjson::Value& value, SettingGroupDef* groupDef) {
                ...
                if (value.HasMember("children"))
                {
                        ...
                        child != children.MemberEnd(); child++)
                    {
                        ...
                        fillItemCall(childValue, settingItemDef, groupDef->label);
                    }
                }
            };
