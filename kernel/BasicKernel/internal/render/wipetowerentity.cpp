...
namespace creative_kernel {
	...
	void WipeTowerEntity::setPrinterBox_Position(QString& _type, const qtuser_3d::Box3D& box)
	{
		m_printerBox = box;
		if (_type=="Left Upper")
		{
			m_position = QVector3D(box.min.x(),box.max.y(),0.0);
		}
		//else if (_type == "Middle Upper")
		//{
		//	m_position = QVector3D(box.center().x(), box.max.y(), 0.0);
		//}
		else if (_type == "Right Upper")
		{
			m_position = QVector3D(box.max.x(), box.max.y(), 0.0);
		}
		else if (_type == "Left Center")
		{
			m_position = QVector3D(box.min.x(), box.center().y(), 0.0);
		}
		else if (_type == "Middle Center")
		{
			m_position = QVector3D(box.center().x(), box.center().y(), 0.0);
		}
		else if (_type == "Right Center")
		{
			m_position = QVector3D(box.max.x(), box.center().y(), 0.0);
		}
		else if (_type == "Left Below")
		{
			m_position = QVector3D(box.min.x(), box.min.y(), 0.0);
		}
		else if (_type == "Middle Below")
		{
			m_position = QVector3D(box.center().x(), box.min.y(), 0.0);
		}
		else if(_type == "Right Below")
		{
			m_position = QVector3D(box.max.x(), box.min.y(), 0.0);
		}
		else
		{
			//Middle Upper
