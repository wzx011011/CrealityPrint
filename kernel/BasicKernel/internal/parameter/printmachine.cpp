...
namespace creative_kernel
{
    ...
    void PrintMachine::added(bool isInitialize)
    {
        ...
        bool isUserBase = m_data.inherits_from == "" && m_data.isUser;
        ...
        for (int i = 0; i < count; ++i)
        {
            ...
            for (const QString& selectMaterialName : selectMaterialNames)
            {
               ...
               if (selectMaterialName == materialUName)
               {
                   MaterialData data;
                   data.id = meta.id;
    ...
    void PrintMachine::syncExtruders(const QStringList& rfid, const QVariantList& color, QList<QVariantMap> data)
    {
        ...
        std::copy_if(data.begin(), data.end(), std::back_inserter(filteredData), [this](const QVariantMap& map) {
         ...
         m_SyncExtruderInfos.clear();
        for (int index = 0;index < std::min(filteredData.size(), m_extruders.size());index++)
        {
    ...
    {
        int index = -1;
        QString material_unique_name = materialName.contains("-1.75") ? materialName : materialName + "-1.75";
        for (int i = 0; i < m_materials.size(); ++i)
        {
            const auto& data = m_materials[i];
            //if (data->name() == material_unique_name)
            if (data->uniqueName() == material_unique_name)
            {
                index = i;
                if (m_materialsModel) {
    ...
    void PrintMachine::finishMeterialSelect(const int& index)
    {
        auto func = [this](MaterialData* mData) {
            ...
            for (const MaterialData& meta : metas)
            {
                ...
                {
                    material = new PrintMaterial(uniqueName(), meta, this);
                    break;
                }
            }
            return material;
        ...
        std::sort(m_materials.begin(), m_materials.end(), [this](const PrintMaterial* m1, const PrintMaterial* m2){
            int rank1 = m1->rank();
            int rank2 = m2->rank();
            return rank1 > rank2;
            });
        ...
        {
            PrintMaterial* material = new PrintMaterial(uniqueName(), um, this);
            material->addedUserMaterial();
    ...
    void PrintMachine::addUserMaterialToJson(const MaterialData& materialMeta)
    {
        //需要实现
	...
	MaterialData PrintMachine::findMetaByName(const QString& machineName)
    {
        ...
        for (const MaterialData& meta : metas)
        {
            if (machineName == meta.uniqueName())
            {
                res = meta;
                break;
            }
