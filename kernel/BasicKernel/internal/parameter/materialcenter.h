...
namespace creative_kernel
{
	...
	public:
		...
		Q_INVOKABLE QVariantList getFlushingVolumes(const QVariantList& hexColor, const QVariantList& types,float flush_multiplier = 1.0);
	...
	public:
		void addExtruder(const QList<QColor>& hexColor,const QList<QString>& types);
		void removeExtruder();
		void setExtruderColor(const QList<QColor>& hexColor,const QList<QString>& types,int index);
	signals:
		void materialsChanged();
		void dirtied();
	};

	void calculate_flush_matrix_and_set(const QString& materialColors, QString& materialTypes,const QString& flush_matrix, const QString& flush_vector, float flush_multiplier);
}
