...
namespace creative_kernel
{
    ...
    void ParameterManager::initialize()
    {
        ...
        for (const QString &machineUniqueName : machineUniqueNames)
        {
            ...
            if (machine)
            {
                ...
                ParameterDataItem* phItem = machine->getDataModel()->findOrMakeDataItem("printable_height");
                ParameterDataItem* paItem = machine->getDataModel()->findOrMakeDataItem("printable_area");

                connect(phItem, &ParameterDataItem::valueChanged, this, [=]() {
                    applyPrintMachine(machine);
                    }); 
                connect(paItem, &ParameterDataItem::valueChanged, this, [=]() {
                    applyPrintMachine(machine);
                    });
            }
        }
    ...
    void ParameterManager::addUserMachine(QObject* machineTemplate, const QStringList& materials, const QStringList& processes)
    {
        ...
        for (const auto& entry : material_template_dir.entryInfoList(QDir::Files | QDir::NoDotAndDotDot)) {
            ...
            if (materials.contains(material_name))
            {
                ...
                const QList<MaterialData>& ml = mc->materialMetas();
                for (auto value : ml)
                {
                    if (value.name == material_name)
                    {
                        data = value;
                    }
                }
    ...
    void ParameterManager::setCurrentMachine(int index, bool needEmit)
    {
        ...
        if (m_currentMachine)
        {
            syncExtruder(new_machine, m_currentMachine);
        }
    ...
    bool ParameterManager::isCrealityMachine()
    {
        PrintMachine* machine_src = currentMachine();
        if (!machine_src)
        {
            return false;
        }
        //for (auto machine : m_machines)
        for (auto machineName : m_factoryMachineNames)
        {
            if (machineName == machine_src->inheritsFrom())
            {
                return true;
            }
        }
    ...
    void ParameterManager::saveCurrentProjectSettings(const QString& filename)
    {
        ...
        auto funcAddString = [&needEscape, &document, &allocator](ParameterBase* machine, QStringList machine_parameter_keys) {
            for (const auto& key : machine_parameter_keys)
            {
                ...
                {
                    PrintMachine* cmachine = qobject_cast<PrintMachine*>(machine);
                    document.AddMember("curr_bed_type", rapidjson::Value(cmachine->currBedType().toStdString().c_str(), allocator), allocator);
                    continue;
                }
    ...
    void ParameterManager::loadProjectSettings(const QString& filename,std::vector<float>& wipe_tower_x,std::vector<float>& wipe_tower_y,bool& bOfficial)
    {
        MachineData data;
        ...
        {
            QString official_printer_name = printer_model;
            if(official_printer_name.indexOf("Creality ") == 0)
            {
                official_printer_name = official_printer_name.mid(9);
            }
            if (machine_data.codeName == printer_model || machine_data.baseName == official_printer_name)
            {
                baseName = machine_data.baseName;
                printer_model = machine_data.codeName;
                bOfficial = true;
                break;
            }
        }
        if (!bOfficial &&baseName != printer_settings_id)
        {
        ...
        if(machine_exist && !data.is_import&&document.HasMember("machine"))
        {
            ...
            if (document.HasMember("extruder") && document["extruder"].IsArray())
            {
                ...
                for (size_t i = 0; i < extruder_array.Size(); i++)
                {
                    ...
                    if (extruder_object.HasMember("material") && extruder_object["material"].IsObject())
                    {
                        ...
                        if (!QFile(defaultMaterialFile(data.uniqueName(), material_name, false)).exists())
                        {
                            docJsonFile.SetObject();
                            ...
                            QFile material_json_file(defaultMaterialFile(data.uniqueName(), material_name, true));
            ...
            if (document.HasMember("process") && document["process"].IsObject())
            {
                ...
                if (process_object.HasMember("factory_profile") && process_object["current_profile"].IsString())
                {
                    factory_profile = process_object["factory_profile"].GetString();
                }
                if (!QFile(defaultProfileFile(machine_unique_name, process_name + ".json")).exists())
                {
                    docJsonFile.SetObject();
                    ...
                    QString process_file_name = userProfileFile(data.uniqueName(), default_print_profile+".json");
                    qDebug() << process_file_name;
                    rapidjson::StringBuffer strbufProcess;
                    rapidjson::Writer<rapidjson::StringBuffer> writerProcess(strbufProcess);
                    docJsonFile.Accept(writerProcess);
                ...
                if (process_object.HasMember("different_settings_to_factory") && process_object["different_settings_to_factory"].IsObject()&&QFile(defaultProfileFile(machine_unique_name, process_name + ".json")).exists())
                {
            ...
            _addMachineFromUniqueName(data.uniqueName(),true);
        ...
        for (int i = 0; i < 1; i++)
        {
            ...
            for (const auto& key : extruder_parameter_keys)
            {
                ...
                if (document.HasMember(ckey) && document[ckey].IsArray())
                {
                    const auto& doc = document[ckey];
                    if (!doc.Empty())
                    {
                        const auto& array = doc[0];
    ...
    void ParameterManager::_addMachine(PrintMachine *machine,bool keepSettings)
    {
        if (!machine)
            return;
        ...
        if(!keepSettings)
            removeCacheMachine(machine_name);
        makeSureUserParamPackDir(machine_name);
        if (m_currentMachine && !keepSettings)
        {
    ...
    void ParameterManager::_addMachineFromUniqueName(const QString &uniqueName,bool keepSettings)
    {
        ...
        _addMachine(machine,keepSettings);
        machine->filterMeterialsFromTypes();
    ...
    void ParameterManager::_setCurrentMachine(PrintMachine *machine, bool needEmit)
    {
        ...
        if (m_currentMachine)
        {
            ...
            for (auto index = 0; index < extruders.size(); ++index)
            {
                ...
                disconnect(extruder, &PrintExtruder::typeChanged, this, &ParameterManager::onExtruderColorChanged);
            }
        ...
        for (auto index = 0; index < extruders.size(); ++index)
        {
            ...
            connect(extruder, &PrintExtruder::typeChanged, this, &ParameterManager::onExtruderColorChanged);
            hexColor.push_back(extruder->color());
            types.push_back(extruder->curMaterial()?extruder->curMaterial()->type():"PLA");
        }
        m_materialCenter->setFlushingParams(m_materialCenter->getFlushingVolumes(hexColor,types));
    ...
    void ParameterManager::_removeMachine(PrintMachine *machine)
    {
        ...
        if (m_currentMachine == machine)
        {
            ...
            if (m_machines.count() > 0)
            {
                ...
                syncExtruder(nextMachine, m_currentMachine);
                machine->deleteLater();
            }
            else
            {
    ...
    void ParameterManager::onExtruderAdded(QObject* extruder)
    {
        ...
        for (auto index = 0; index < extruders.size(); ++index)
        {
            auto* extruder = extruders.at(index);
            hexColor.push_back(extruder->color());
            types.push_back(extruder->curMaterial()->materialType());
        }

        m_materialCenter->addExtruder(hexColor,types);
    }
    ...
    void ParameterManager::onExtruderColorChanged()
    {
        ...
        m_materialCenter->setExtruderColor(hexColor, types,extruder->index());
    }

    void ParameterManager::syncExtruder(PrintMachine* newMachine, PrintMachine* oldMachine)
    {
        int old_extruder_count = oldMachine->extruderAllCount();
        int new_extruder_count = newMachine->extruderAllCount();
        while (new_extruder_count > old_extruder_count)
        {
            newMachine->removeExtruder();
            --new_extruder_count;
        }
        for (int i = 0; i < oldMachine->extruderAllCount(); ++i)
        {
            auto extruder = oldMachine->extruders()[i];
            if (new_extruder_count <= i)
            {
                newMachine->addExtruder();
            }
            if (newMachine->extruders().size() == i)//add failed
            {
                break;
            }
            auto new_extruder = newMachine->extruders()[i];
