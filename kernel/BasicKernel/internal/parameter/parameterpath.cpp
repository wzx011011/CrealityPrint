...
namespace creative_kernel
{
    ...
    void loadMaterialMeta(QList<MaterialData>& metas, const bool& user, const QString& machineName)
    {
        ...
        if(user)
        {
            QList<MaterialData> sysmetas;
            loadMaterialMeta(sysmetas);
            for (auto m : sysmetas)
			    metaNameCache.insert(m.name);
        }
        ...
        for (const QJsonValueRef& materialRef : materials) {
            ...
            if(user)
            {
                QString material_file_path = QString("%1/%2/Materials/%3-1.75.json").arg(_pfpt_user_parampack).arg(machineName).arg(name);
                if(!QFileInfo(material_file_path).exists())
                {
                    continue;
                }
            }
