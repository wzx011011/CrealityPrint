...
namespace creative_kernel
{
	...
	void ParameterBase::setUserSettings(us::USettings* settings)
	{
		...
		if (m_settings) {
			const QHash<QString, us::USetting*>& settings = m_user_settings->settings();
			auto it = settings.begin();
			while (it != settings.end())
			{
				if (m_settings->hasKey(it.key()))
				{
					if (m_settings->value(it.key(), "") == it.value()->str())
					{
						m_user_settings->deleteValueByKey(it.key());
					}
				}
				it++;
			}
