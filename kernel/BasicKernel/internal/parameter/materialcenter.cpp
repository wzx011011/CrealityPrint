...
namespace creative_kernel
{
	...
	QVariantList MaterialCenter::getFlushingVolumes(const QVariantList& hexColor,const QVariantList& types, float flush_multiplier)
	{
		...
		for (int from_idx = 0; from_idx < m_colours.size(); from_idx++) {
			for (int to_idx = 0; to_idx < m_colours.size(); to_idx++) {
				if(types[from_idx].toString() != types[to_idx].toString())
				{
					if(matrix[from_idx * m_colours.size() + to_idx] < nozzle_volume()*3)
						matrix[from_idx * m_colours.size() + to_idx] = nozzle_volume()*3;
				}
			}
		}
	...
	void MaterialCenter::addExtruder(const QList<QColor>& hexColor,const QList<QString>& types)
	{
		std::vector<trimesh::vec3> m_colours;
			...
			for (int i = 0; i < _matrix_size; i++) {
				...
				for (int j = 0; j < _matrix_size; j++)
				{
					...
					if (i == _matrix_size - 1 || j == _matrix_size - 1)
					{
						...
						if(types[i]!=types[j])
						{
							if(_value<nozzle_volume()*3)
							{
								_value = nozzle_volume()*3;
							}
						}
					}
					else
	...
	void MaterialCenter::setExtruderColor(const QList<QColor>& hexColor, const QList<QString>& types,int index)
	{
		std::vector<trimesh::vec3> m_colours;
		for (const auto& color : hexColor) {
			trimesh::vec3 color1;
	...
	void calculate_flush_matrix_and_set(const QString& materialColorsString, QString& materialTypes,const QString& flushMatrixString, const QString& flushVectorString, float flush_multiplier)
	{
		auto globalSetting = currentGlobalUserSettings();
		QString matrixString;
