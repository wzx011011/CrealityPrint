...
namespace creative_kernel
{
    ...
    void Load3MFJob::successed(qtuser_core::Progressor* progressor)
    {
        ...
        getKernel()->parameterManager()->loadProjectSettings(m_projectConfigFile,wipe_tower_x,wipe_tower_y,bOfficial);
        ...
        for (int i = 0; i < printer_count; ++i)
        {
            ...
            if (i < plates.size())
            {
                ...
                ptr->applySettings(createCurrentGlobalSettings());
            }
        }
        ...
        QString materialTypes = settings->vvalue("filament_type").toString();
        ...
        calculate_flush_matrix_and_set(materialColors, materialTypes,flush_matrix, flush_vector, maltip);
        ...
        if(!bOfficial)
        {
            requestQmlDialog("open3MFWarningDialog");
        }
    }
    void Load3MFJob::processSceneData()
    {
        getKernel()->meshLoadCenter()->process(m_scene);
    }
    void Load3MFJob::work(qtuser_core::Progressor* progressor)
    {
        qtuser_core::ProgressorTracer tracer(progressor);

        common_3mf::Read3MF* reader = new common_3mf::Read3MF(cxkernel::qString2String(m_fileName),projectUI()->getAutoProjectDirPath().toStdString());
        m_scene_create_data.add_groups.clear();
        m_scene_create_data.remove_groups.clear();

        tracer.progress(0.3f);
        
        if (reader->read_all_3mf(m_scene, false,&tracer))
        {
            m_read3mfState = true;
        ...
        {
            delete reader;
            reader = new common_3mf::Read3MF(cxkernel::qString2String(m_fileName),projectUI()->getAutoProjectDirPath().toStdString());
            if(reader->read_all_3mf(m_scene, true,&tracer))
            {
                m_read3mfState = true;
    ...
    void Load3MFJob::createSceneData()
    {
        ...
        //集中创建ModelDataPtr
        std::vector<common_3mf::Model3MF*> model_map;
        for (common_3mf::Group3MF& group : m_scene.groups)
        {
            for (common_3mf::Model3MF& model : group.models)
            {
                model_map.push_back(&model);
            }
        }
        tbb::concurrent_hash_map<void*, ModelDataPtr> model_data_map;
        tbb::parallel_for(tbb::blocked_range<int>(0, model_map.size()), [&](const tbb::blocked_range<int>&range){
            for (int i = range.begin(); i != range.end(); ++i)
            {
                common_3mf::Model3MF& model = *model_map[i];
                ...
                model_data_map.insert(std::make_pair((void*)&model, model_data));
            }
        });
        //创建场景数据结构
        for (common_3mf::Group3MF& group : m_scene.groups)
        {
            ...
            for (common_3mf::Model3MF& model : group.models)
            {
                ...
                //int extruder = model.extruder > 0  ? model.extruder - 1 : 0;
                ModelDataPtr model_data = nullptr;
                tbb::concurrent_hash_map<void*, ModelDataPtr>::const_accessor accessor;
                if(model_data_map.find(accessor,&model))
                {
                    model_data = accessor->second;
                }                
                if (!model_data)
                    continue;
    ...
    std::vector<MeshWithName> load_meshes_from_3mf(const QString& file_name, ccglobal::Tracer* tracer)
    {
        ...
        if (reader.read_all_3mf(scene, false,tracer))
        {
