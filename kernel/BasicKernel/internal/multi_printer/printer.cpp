...
namespace creative_kernel {
	...
	void Printer::updateBox(const qtuser_3d::Box3D& box)
	{
		...
		bool isPreparePhase = phase == 0 || phase == -1;
		//bool isPreparePhase = true;

		if (isPreparePhase)
		{
			QString  _type = WipeTowerDefaultPosition();
			WipeTowerEntity* tower = m_entity->wipeTowerEntity();
			tower->setPrinterBox_Position(_type, box);
		}
	...
	QString Printer::machineBaseName()
	{
		PrintMachine* currentMachine = getKernel()->parameterManager()->currentMachine();
		return currentMachine->inheritName();
	}
	...
	QString Printer::WipeTowerDefaultPosition()
	{
		PrintMachine* currentMachine = getKernel()->parameterManager()->currentMachine();
		if (currentMachine != nullptr)
		{
			QString type = currentMachine->settingsValue("prime_tower_position_type");
			return type;
		}
		return "Middle Upper";
	}

	void Printer::WipeTowerInitPosition()
	{
		WipeTowerEntity* tower = m_entity->wipeTowerEntity();
		updateBox(tower->printerBox());
	}
	...
	bool Printer::checkBedTypeInvalid() {
		...
		for (auto index : extruder_indexes) {
			...
			auto* material = extruder->curMaterial();
			if (!material) {
				continue;
			}
