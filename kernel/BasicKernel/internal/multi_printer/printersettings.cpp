...
namespace creative_kernel
{
    ...
    void PrinterSettings::reloadHeightLayerMap()
    {
        ...
        {
            trimesh::vec3 pos = attain->traitPosition(i + 1, 1);
            float height = pos.z;
    ...
    float PrinterSettings::layerHeight(int layer)
    {
        ...
        if (attain->layers() < layer)
         ...
         trimesh::vec3 pos = attain->traitPosition(layer + 1, 1);
    ...
    void PrinterSettings::applySettings(QSharedPointer<us::USettings> settings)
    {
        ParameterManager* parameterManager = getKernel()->parameterManager();
        {
            QString value = m_settings[BED_TYPE];
            if (!value.isEmpty())
                settings->add("curr_bed_type", value,true);
        }

        {
            QString value = m_settings[SEQUENCE];
            if (!value.isEmpty())
                settings->add("print_sequence", value,true);
        }
        ...
        if (m_settings[CUSTOM] == "true")
        {
            ...
            if (!value.isEmpty())
                settings->add("first_layer_print_sequence", value,true);
            else
                settings->add("first_layer_print_sequence", "0",true);
        }
        else 
        {
            settings->add("first_layer_print_sequence", "0",true);
