...
;		p->replaceModelGroupsInside(groups);
	...
	void PrinterMangager::clearAllPrinterSettings()
	{
		for (Printer* p : m_printers)
		{
			p->clearSettings();
	...
	void PrinterMangager::divideModelGroupsForPrinter(Printer* printer, QList<ModelGroup*>& groups, QList<int>& groupedFlags)
	{
		...
		for (size_t j = 0; j < groups.size(); j++)
		{
			...
			{
				if(fabs(modelBox.min.x() - printerBox.min.x())<0.0001)
				{
					modelBox.min.setX( printerBox.min.x());
				}
				if(fabs(modelBox.min.y() - printerBox.min.y())<0.0001)
				{
					modelBox.min.setY( printerBox.min.y());
				}
				if(fabs(modelBox.max.x() - printerBox.max.x())<0.0001)
				{
					modelBox.max.setX( printerBox.max.x());
				}
				if(fabs(modelBox.max.y() - printerBox.max.y())<0.0001)
				{
					modelBox.max.setY( printerBox.max.y());
				}
				
				if(fabs(modelBox.max.z() - printerBox.max.z())<0.0001)
				{
					modelBox.max.setZ( printerBox.max.z());
				}
	...
	void PrinterMangager::updateWipeTowersInitPosition()
	{
		for (auto p : m_printers)
		{
			p->WipeTowerInitPosition();
		}
	}
