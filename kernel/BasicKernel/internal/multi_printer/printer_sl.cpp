...
namespace creative_kernel {
	...
	void Printer::triggerCapture()
	{
		auto captureCallback = [=](QObject* receiver, const QImage& image)
		{
            ...
            if (printer)
			{
				...
				if (!isValid) 
				{
					qWarning() << QString("Capture a invalid picture on plate %1! Retry %2 times!!!!!").arg(printer->index()).arg(m_tryCaptureCount);
					if (m_tryCaptureCount > 0)
						m_requestPictureTimer.start(500);
				}
		...
		qWarning() << QString("******* Capture plate %1! model count: %2 ").arg(index()).arg(modelsInside().size());
	...
	void Printer::applySettings(QSharedPointer<us::USettings> settings)
	{
		m_settings->applySettings(settings);
		settingsApplyed();
	}
