...
namespace creative_kernel
{
    ...
    {
        m_triggerSaveTimer.setSingleShot(true);
        connect(&m_triggerSaveTimer, &QTimer::timeout, this, &CX3DManager::onTriggerSave);
    }
    ...
    {
        qtuser_core::VersionSettings setting;
        int typeIndex = setting.value("projectTypeIndex", 0).toInt();
 ...
 /*       QString filePath = defaultPath().toLocalFile();
    ...
    {
        m_saveRequest.groups = modelgroup;
        m_saveRequest.type = type;
        m_triggerSaveTimer.stop();
        m_triggerSaveTimer.start(1000);
    }

    void CX3DManager::onTriggerSave()
    {
        m_autoCommand->triggerAutoSave(m_saveRequest.groups, m_saveRequest.type);
    }
    ...
    void CX3DManager::exportAllPlate()
    {
        ...
        bool res = m_exportcommand->exportGcode3mf(gcode3mfname, getSlicedPrinterIndice());
        if (res)
        {
            m_strMessageText = QCoreApplication::translate("creative_kernel::SliceFlow", "Save Finished, Open Local Folder");
