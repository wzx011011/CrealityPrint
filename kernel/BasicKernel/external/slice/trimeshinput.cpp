...
namespace boost
{
	namespace serialization
	{
		...
		void serialize(Archive& ar, trimesh::TriMesh::Face& f, const unsigned int version)
		{
			ar& f.x;
			ar& f.y;
			ar& f.z;
	    ...
	    //序列化顶点数据
		template <typename Archive>
		void serialize(Archive& ar, trimesh::point& v, const unsigned int version)
		{
			ar& v.x;
			ar& v.y;
			ar& v.z;
...
{
	trimesh::TriMesh* mesh = m_model->mesh(true);
#ifdef _EXPORT_SERIAL_DATA
	//	//序列化结果
	//std::string outDataFile = std::string("d:/outdir/421-1.dat");
	//std::ofstream ofs(outDataFile);
	//boost::archive::binary_oarchive ob(ofs);
	//ob << mesh->faces;
	//ob << mesh->vertices;
#endif
...
float* TrimeshInput::offset() const
{
	if (m_offset != nullptr && m_offset != trimesh::vec3())
...
us::ModelSetting* TrimeshInput::modelSetting()
{
	return m_model->setting();
}

const us::ModelSetting* TrimeshInput::modelSetting() const
{
	return m_model->setting();
}
...
trimesh::TriMesh* TrimeshInput::generateBeltSupport(trimesh::TriMesh* mesh,float angle)
{
	...
	*mesh_temp = *mesh;
	QMatrix4x4 globalMatrix = m_model->globalMatrix();
	trimesh::fxform xf1 = qMatrix2Xform(globalMatrix);
