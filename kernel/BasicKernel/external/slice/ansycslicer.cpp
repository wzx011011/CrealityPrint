...
namespace creative_kernel
{
	...
	{
		input.G =  SettingsPointer(createCurrentGlobalSettings()->copy());

		QString print_sequence = input.G->value("print_sequence", "");
		qWarning() << "print_sequence: " << print_sequence;
		printer->applySettings(input.G);
		set_software_version(input.G.data(), version());
		QString printer_model = printer->machineBaseName();
		if(QFile::exists(QString("%1/%2/%2.def.json").arg(_pfpt_default_parampack).arg(printer_model)))
		{
			printer_model = printer_model.mid(0, printer_model.lastIndexOf("-"));;
			input.G->add("printer_model",getMachineCodeName(printer_model),true);
		}
