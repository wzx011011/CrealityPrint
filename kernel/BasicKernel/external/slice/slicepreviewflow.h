...
namespace creative_kernel
{
	...
	class SlicePreviewFlow : public QObject
	{
		...
		// Q_PROPERTY(QString printerName READ printerName NOTIFY speedChanged);
	...
	public:
		...
		void notifyLayerRange();

		Q_INVOKABLE void setGCodeVisualType(int type);	// preview type
		// Q_INVOKABLE void setIndicatorVisible(bool visible);
		// Q_INVOKABLE void setPrinterVisible(bool visible);
		Q_INVOKABLE void showGCodeType(int type, bool show);	// component type

		void setCurrent(int layer, int step);
		...
		void setPreviewNode(GCodePreviewNode* node);

	private slots:
		void onPreviewNodeDestroyed();
	...
	private:
		...
		QTimer m_requestLayerGCodeTimer;
		int m_requestLayer{0};

		bool m_roleState[(int)Orca::ExtrusionRole::Count];

		/* gcode */
		QList<QVector4D> m_gcodeComponentColors;
		Qt3DCore::QEntity* m_root;
		GCodePreviewEffect* m_gcodeEffect{ NULL };
		GCodePreviewEffect* m_gcodeMonoEffect{ NULL };
		GCodePreviewNode* m_previewNode{ NULL };

		bool m_showOnlyTopLayer{ false };
