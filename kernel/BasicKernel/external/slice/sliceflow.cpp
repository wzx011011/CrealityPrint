...
namespace creative_kernel
{
          ...
          }() }
    {
        ...
        getKernel()->visScene()->setPreviewScene(m_slicePreviewFlow->sceneRoot());
    ...
    void SliceFlow::onParseGcodeSuccuess(SliceAttain* attain)
    {
        ...
        if (box.min == box.max)
            box = getPrinter(0)->printerBox();
		creative_kernel::updatePrinterBox(box);
    }

    void SliceFlow::saveTempGCodeSuccess()
    {
        auto attain = m_slicePreviewFlow->attain();
        if (attain)
        {
            attain->saveGCodeImage();
            emit sliceAttainChanged();
        }
    }
    ...
    {
        qWarning() << "SliceFlow::handle:"<<fileName<<":"<<m_slicePreviewFlow->attain();
    ...
    QString SliceFlow::gcodeThumbnail()
    {
        ...
        if (m_slicePreviewFlow->attain())
        {
            ...
            {
                QUrl url = QUrl::fromLocalFile(m_slicePreviewFlow->attain()->tempGcodeThumbnail());
                name = url.toString();
            }
    ...
    {
		setModelOffset(QVector3D(0, 0, 0));
    }
    ...
    QUrl SliceFlow::getExportUrl()
    {
        QString localPath = getExportName();
        QUrl fileName = QUrl::fromLocalFile(localPath);
        return fileName;
    }
    QString SliceFlow::getExportName()
    {
        ...
        if (attain)
        {
			...
			if (attain->isFromGcode())
			{
                ...
                QString exportNameFormat = attain->outputnameFormat();
                QString input_filename_base=gcodeName;
                QString printer_model = getKernel()->parameterManager()->currentMachine()->baseName();
                int printTime = attain->printTime();
                auto calcTime = [](int time)->QString {
                    if(time>24*3600)
                    {
                        int day = time/(24*3600);
                        int hour = (time%(24*3600))/3600;
                        int minute = (time%3600)/60;
                        return QString("%1d%2h%3m").arg(day).arg(hour).arg(minute);
                    }
                    else if(time>3600)
                    {
                        int hour = time/3600;
                        int minute = (time%3600)/60;
                        return QString("%1h%2m").arg(hour).arg(minute);
                    }
                    else
                    {
                        return QString("%1m").arg(time/60);
                    }
                };
                QString print_time = calcTime(printTime);
                int initial_tool = m->getFirstItemMaterialIndex();
                exportName = exportNameFormat.replace("{input_filename_base}", input_filename_base);
                exportName = exportName.replace("initial_tool", QString::number(initial_tool));
                exportName = exportName.replace("{printer_model}", printer_model);
                exportName = exportName.replace("{print_time}", print_time);
                if(exportNameFormat.indexOf("initial_tool")>0)
                    exportName = exportName.replace(QString("{filament_type[%1]}").arg(initial_tool), currentMaterialsType(initial_tool));
                else
                {
                    initial_tool = 0;
                    QRegularExpression re("\\[(\\d+)\\]");
                    QRegularExpressionMatch match = re.match(exportName);
                    if (match.hasMatch()) {
                        QString number = match.captured(1);
                        initial_tool = number.toInt();
                    }
                    exportName = exportName.replace(QString("{filament_type[%1]}").arg(initial_tool), currentMaterialsType(initial_tool));
                }
                QFileInfo fileInfo(exportName);
