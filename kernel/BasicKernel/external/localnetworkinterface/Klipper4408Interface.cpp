...
namespace creative_kernel
{
	...
	void Klipper4408Interface::updateAllUploadState(RemotePrinter::UploadStatus status)
	{
		 for (auto it = m_webSockets.begin(); it != m_webSockets.end(); ++it) {
			it->second->updateUploadState(status);
    	}
	}
	...
	std::future<void> Klipper4408Interface::sendFileToDevice(const std::string& strServerIp, const int& port, const std::string& fileName, const std::string& filePath, std::function<bool(float)> callback, std::function<void(int)> errorCallback)
	{
			...
			cpr::ProgressCallback([=](cpr::cpr_off_t downloadTotal, cpr::cpr_off_t downloadNow, cpr::cpr_off_t uploadTotal, cpr::cpr_off_t uploadNow, intptr_t userdata) {
				if (uploadTotal && uploadNow)
				{
					if (callback != nullptr)
					{
						...
						{
							bCancel = callback(0.99f);
						}
						else
						{
							bCancel = callback((float)uploadNow / uploadTotal);
						} 
						return !bCancel;
					}
				}
