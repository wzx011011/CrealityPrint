...
namespace creative_kernel
{
	...
	void RemotePrinterManager::pushFile(const RemotePrinter& printer, const std::string& fileName, const std::string& filePath, std::function<void(std::string, std::string, float)> callback, std::function<void(std::string, std::string, int)> errorCallback)
	{
		switch (printer.type)
		{
			...
			case RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER:
			{
				auto future = m_pKlipperInterface->sendFileToDevice(printer.ipAddress.toStdString(), printer.moonrakerPort, fileName, filePath, 
					[=](float progress)-> bool {
						...
						if(RemotePrinter::UploadStatus::UPLOAD_STATUS_CANCELED == printer.uploadStatus)
						{
							return true;
						}
						return false;
			...
			case RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408:
			{
				m_pKlipper4408Interface->updateAllUploadState(RemotePrinter::UPLOAD_STATUS_UPLOADING);
				auto future = m_pKlipper4408Interface->sendFileToDevice(printer.ipAddress.toStdString(), 80, fileName, filePath,
					[&printer,callback,fileName](float progress)-> bool {
				...
				m_pKlipper4408Interface->updateAllUploadState(RemotePrinter::UPLOAD_STATUS_FINISHED);
	...
	void RemotePrinterManager::uploadFile(RemotePrinter& printer, const std::string& fileName, const std::string& filePath, std::function<void(std::string, std::string, float)> callback, std::function<void(std::string, std::string, int)> errorCallback)
	{
		m_mtxUpload.lock();
		printer.uploadStatus = RemotePrinter::UploadStatus::UPLOAD_STATUS_PENDING;
	...
	void creative_kernel::RemotePrinterManager::uploadThread()
	{
		while (!m_bExit)
		{
			...
			m_mapUploadsStatus.insert(printer.ipAddress,&printer);
			//printer.uploadCanceled = false;
			if(printer.uploadStatus==RemotePrinter::UploadStatus::UPLOAD_STATUS_CANCELED)
			{
				continue;
			}
			printer.uploadStatus= RemotePrinter::UploadStatus::UPLOAD_STATUS_UPLOADING;
			
			pushFile(printer,filename,filePath,call_back,error_call_back);
			if(printer.uploadStatus==RemotePrinter::UploadStatus::UPLOAD_STATUS_CANCELED)
			{
				deleteFile(printer.ipAddress.toStdString(), filename, printer.type, OpPrinerFileType::GCODE_FILE);
			}
			printer.uploadStatus = RemotePrinter::UploadStatus::UPLOAD_STATUS_FINISHED;
			
		}
	}
	void creative_kernel::RemotePrinterManager::cancelUploadFile(QString printerIp)
	{
		if(m_mapUploadsStatus.contains(printerIp))
		{
			m_mapUploadsStatus[printerIp]->uploadStatus = RemotePrinter::UploadStatus::UPLOAD_STATUS_CANCELED;
		}
	}
