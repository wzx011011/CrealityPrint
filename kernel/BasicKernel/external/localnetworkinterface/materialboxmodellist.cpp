...
{
	if (m_materialBoxItemList.size() == 0 || m_bSelectRack)return 1; //只使用外挂耗材，永远使能
	for (MaterialBoxModelItem* item : m_materialBoxItemList)
	{
		if(item->selected())
		{
	...
	for (auto item : m_materialBoxItemList) {
			...
			//}else{
				...
				for (auto it = matchedIndexs.cbegin(); it != matchedIndexs.cend(); ++it) {
					...
					double currentDistance = calculateColorDistance(targetColor, currentColor);
					//如果当前颜色比已匹配的颜色更匹配且比当前匹配喷嘴的颜色更匹配
					if (currentDistance < it.value() && currentDistance < closestDistance) {
						//查找到更匹配的，删除原来的匹配
						int agianIndex = -1;
...
void MaterialBoxListModel::onGetMaterialBoxList(const std::string& std_macAddr, const BoxsInfo& std_list, RemotePrinerType remoteType)
{
		...
		if (box1.materials.size() > 0) {
			...
			else
			{
				for (Material item : box1.materials) {
					...
					//if (box1.humidity > 0)
					modelItem->setHumidity(QString::number(box1.humidity));
	...
	emit materialCountChanged();
}
QObject* MaterialBoxListModel::getGroupHumidityItem(const int groupIndex)
{
	int index = groupIndex*4;
	if (m_materialBoxItemList.size() > index)
	{
		return m_materialBoxItemList.at(index);
	}
	return nullptr;
}
int MaterialBoxListModel::getMaterialCount() const
{
	return m_materialBoxItemList.size();
}
...
QString MaterialBoxModelItem::humidity() const
{
	return m_humidity;
}

void MaterialBoxModelItem::setHumidity(QString humidity)
{
	m_humidity = humidity;
