...
namespace creative_kernel
{
	...
	void WsClient::onConnect(beast::error_code ec, tcp::resolver::results_type::endpoint_type ep)
	{
		...
		int handshakeTimeout = m_printer.uploadStatus==RemotePrinter::UPLOAD_STATUS_UPLOADING?40:20;
		int idleTimeout = m_printer.uploadStatus==RemotePrinter::UPLOAD_STATUS_UPLOADING?12:6;

		beast::websocket::stream_base::timeout opt{
		std::chrono::seconds(handshakeTimeout),   // handshake timeout
		std::chrono::seconds(idleTimeout),       // idle timeout. no data recv from peer for 6 sec, then it is timeout
	...
	void WsClient::updateUploadState(RemotePrinter::UploadStatus status)
	{
		m_printer.uploadStatus = status;
	}
	...
	void WsClient::onFail(beast::error_code ec, char const* what)
	{
		...
		int connectTimeout = m_printer.uploadStatus==RemotePrinter::UPLOAD_STATUS_UPLOADING?10:5;
		if (m_printer.printerStatus && infoCallback && abs(tmNow - m_offlineTime) > connectTimeout)
		{
	...
	void WsClient::ParseMsg(const std::string& message)
	{
		...
		if (doc.HasMember("targetBoxTemp") && doc["targetBoxTemp"].IsInt())
		{
			m_printer.chamberTempTarget = doc["targetBoxTemp"].GetInt();
		}
		if (doc.HasMember("maxBoxTemp") && doc["maxBoxTemp"].IsInt())
		{
			m_printer.maxBoxTemp = doc["maxBoxTemp"].GetInt();
		}
		...
		if (doc.HasMember("feedState") && doc["feedState"].IsInt())
		{
			...
			int feedState = doc["feedState"].GetInt();
			qWarning() << "feedState:" << feedState;
			if(feedState>0)
				m_printer.feedState = feedState;
			
			if((m_printer.feedState==3||m_printer.feedState==13||m_printer.feedState==14 ||m_printer.feedState==113||m_printer.feedState==107) && feedState==0)
				m_printer.feedState = 0;
