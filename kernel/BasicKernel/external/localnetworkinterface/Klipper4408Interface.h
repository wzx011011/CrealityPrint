...
namespace creative_kernel
{
	...
	class BASIC_KERNEL_API Klipper4408Interface: public QObject
	{
		...
		public:
			...
			std::future<void> sendFileToDevice(const std::string& strServerIp, const int& port, const std::string& fileName, const std::string& filePath, std::function<bool(float)> callback, std::function<void(int)> errorCallback);
			...
			void updateAllUploadState(RemotePrinter::UploadStatus status);
