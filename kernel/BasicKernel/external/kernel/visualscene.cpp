...
namespace creative_kernel
{
		...
		, m_handlerEnabled(true)
	{
		...
		m_delayRequestTimer.setSingleShot(true);
		connect(&m_delayRequestTimer, &QTimer::timeout, this, [=]()
			{
				updateRender(true);
			});
	...
	bool VisualScene::capturing()
	{
		return !m_captureFinished;
	}
	...
	void VisualScene::updatePickDelayed(int delay)
	{
		m_delayRequestTimer.start(delay);
	}
