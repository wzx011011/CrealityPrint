...
namespace creative_kernel
{
	...
	void Kernel::processCrealink(const QString& link)
	{

		if (link.startsWith("crealityprintlink"))
			{
				if(!m_bStarted)
				{
					m_crealityPrinterLink = link;
					return;
				}
				QString url_3mf = link.mid(link.indexOf("=")+1);
				
				QUrl u3mf = QUrl::fromPercentEncoding(url_3mf.toUtf8());

				const auto project_service = cxcloud()->getProjectService().lock();
				if (project_service)
				{
					project_service->importProjectFromUrl(u3mf);
				}
			}
	}
	void Kernel::processCommandLine()
	{
		...
		if(!m_crealityPrinterLink.isEmpty())
		{
			processCrealink(m_crealityPrinterLink);
		}
	...
	void Kernel::viewChanged(bool requestUpdate)
	{
		...
		{
			 if (requestUpdate)
			 	m_visualScene->updatePickDelayed(100);
			 else 
	...
	bool Kernel::is3MFProject(QString filePath)
	{
		return true;
	}
	bool Kernel::isPrintabled3MF(QString fileUrl)
	{
		QUrl url(fileUrl);
		...
		do {
			...
			if (fileName.startsWith("Metadata",Qt::CaseInsensitive)&&fileName.endsWith(".gcode", Qt::CaseInsensitive)) {
				return true;
			}
