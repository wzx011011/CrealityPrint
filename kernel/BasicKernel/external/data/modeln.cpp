...
namespace creative_kernel
{
	...
	void ModelN::updatePalette()
	{
		...
		if (type == ModelNType::normal_part)
		{
			...
			auto softenColor = [=](trimesh::vec3 tColor)
			{
				QVector4D qColor(tColor[0], tColor[1], tColor[2], 1);

				float bn = tColor[0] * 0.3 + tColor[1] * 0.59 + tColor[2] * 0.11;
				float gainFactor = pow((1 - bn * 0.14), 2);	// 根据颜色亮度添加增益，避免暗色系太暗，亮色系太亮。(b - bn * k)中b越大, 暗色系越亮, k越大, 亮色系越暗
				QVector4D gain(gainFactor, gainFactor, gainFactor, 1);
				qColor = qColor * (1 - 0.17) + gain * 0.17;
				return qColor;
			};
			qcolors << softenColor(trimesh::vec3(colors[dIndex][0], colors[dIndex][1], colors[dIndex][2]));
			for (int i = 0; i < 16; ++i)
			{
				trimesh::vec3 color;
				if (i < colors.size())
