...
namespace creative_kernel
{
	...
	void ModelGroup::addModels(const QList<ModelN*>& models)
	{
		for (ModelN* model : models)
		{
			if (!model)
			{
				qWarning() << QString("can not add a invalid model.");
				continue;
			}
			...
			connect(model, &QObject::destroyed, this, &ModelGroup::onComponentModelDestroyed, Qt::UniqueConnection);
	...
	void ModelGroup::removeModels(const QList<ModelN*>& models)
	{
		for (ModelN* model : models)
		{
			...
			disconnect(model, &QObject::destroyed, this, &ModelGroup::onComponentModelDestroyed);
	...
	void ModelGroup::onComponentModelDestroyed()
	{
	...
	std::vector<trimesh::vec3> ModelGroup::caculateOutlinePath()
	{
		...
		{
			ModelN* model = m_componentModels[i];
			if (!model)
				continue;
