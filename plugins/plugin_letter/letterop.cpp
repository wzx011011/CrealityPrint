...
BasicEntity* LetterOp::CreateEntity()
{
	BasicEntity* entity = new BasicEntity();
	entity->setEffect(EFFECTCREATE("finephong", entity));
	entity->createParameter("color", QVector4D(0.5f, 0.0f, 0.0f, 0.7f));
	return entity;
}

void LetterOp::DeleteEntities()
{
	for (BasicEntity* entity : m_vEntities)
	{
		if (entity)
		{
			delete entity;
...
void LetterOp::UpdateTextMesh()
{
	...
	if (m_bShouldUpdateTextModels || m_vMeshes.empty())
	{
		...
		ShapeCreator::createTextMesh(m_strText, m_strTextFont, m_fTextHeight, m_fTextThickness + m_fEmbededDepth, m_vMeshWidths, m_vMeshes);
		...
		for (size_t i = 0; i < m_vMeshes.size(); i++)
		{
			...
			m_vEntities[i]->setGeometry(trimesh2GeometryFace(m_vMeshes[i]));
		}
	}
	...
	for (size_t i = 0; i < m_vMeshes.size(); i++)
	{
		if (!m_vEntities[i] || !m_vEntities[i]->isEnabled() || i >= m_vMeshPoses.size())
		{
			continue;
		}
