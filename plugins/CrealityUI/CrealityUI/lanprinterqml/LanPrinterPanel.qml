...
Item {
    ...
    {
        webrtcSupport
        if(__testLoad.item && __testLoad.item.visible&&_testLoad.item.webrtcSupport!=1)
            __testLoad.item.stopPlayVideo()
    }
    function startPlayVideo()
    {
        if(__testLoad.item && curSelectPrintingIndex >= 0&&_testLoad.item.webrtcSupport!=1)
    ...
    function openDetail(printerID, printerName, printerType)
    {
        __lanPrinterPanel.openDetail(printerID, printerName, printerType)
    }
            ...
            LanPrinterListLocal
            {
                ...
                function openDetail(printerID, printerName, printerType)
                {
                   var result = isModelContains(printerID)
                    ...
                    if(__testLoad.sourceComponent == __test)
                    {
                        __testLoad.item.stopPlayVideo()
                        __testLoad.onLoaded()
                    }
                ...
                onClickDetail:
                {
                    openDetail(printerID, printerName, printerType)
    ...
    LanPrinterDialog {
        ...
        function showMessageDialog(msgType, errorCodeMsg, objectName) {
            ...
            case LanPrinterDetail.MessageType.CancelBrokenMaterial:
                ...
                showCancelBtn = true
                break;
            case LanPrinterDetail.MessageType.InvalidFile:
                title = qsTranslate("LanPrinterDetail", "Invalid file")
                curMessageText = qsTranslate("LanPrinterDetail", "There are no slice files in the currently selected 3mf file, so it cannot be imported")
                showCancelBtn = false
                break;
            case LanPrinterDetail.MessageType.InvalidFiles:
                title = qsTranslate("LanPrinterDetail", "Invalid file")
                curMessageText = qsTranslate("LanPrinterDetail", "There are no slice files in the following 3mf files, so they cannot be imported")
                showCancelBtn = false
                break;
            default:
                break;
