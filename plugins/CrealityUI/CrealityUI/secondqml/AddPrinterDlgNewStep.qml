...
DockItem {
    ...
    onVisibleChanged: {
        ...
        materialCheck = []
        for(let index = 0; index < material_name.count; ++index){
            if(material_name.itemAt(index).checked){
                let txt = material_name.itemAt(index).text;
                if(!materialCheck.includes(txt))materialCheck.push(txt)
            }
        }
    }
                ...
                Grid{
                    ...
                    Repeater {
                        ...
                        delegate: CusCheckBox {
                            ...
                            Component.onCompleted: {
                                if(checked){
                                    if(!materialCheck.includes(model.meName))materialCheck.push(model.meName)
                                }
                            }
                        }
                    }
