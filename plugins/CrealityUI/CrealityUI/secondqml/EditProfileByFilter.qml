...
BasicDialog
{
	...
	{
		var classList = SettingJson.getClassTypeList()
		var paramList = SettingJson.getAllSettingKeyList()

		if(Object.keys(buttonMap).length-1 == classList.length && Object.keys(labelMap).length-1 == paramList.length)
		{
			return true
		}
		else
	...
	function async_createCombobox(key)
	{
		...
		for(var modelKey in modelMap){
            newModel.append({"key":modelKey, "modelData":qsTr(modelMap[modelKey]), "shapeImg":""})
		}
		...
		for(var key in labelMap)
		{
			if(key != 0)
			{
				...
				if(comType == "enum" || comType == "optional_extruder"|| comType == "extruder")
				{
					...
					newModel.clear()
					for(var modelKey in modelMap){
						newModel.append({"key":modelKey, "modelData":qsTr(modelMap[modelKey])})
					}
