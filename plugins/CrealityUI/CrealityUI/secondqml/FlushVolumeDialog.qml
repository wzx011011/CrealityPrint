...
DockItem {
    ...
    property var tipColor2:Constants.textColor
    ...
    function calcTableData(isInit = 0){
        ...
        for(var i =0; i<rowCount;i++){
            ...
            colorArr.push(color)
            typeArr.push(extrudersModel.data(extrudersModel.index(i,0),264))
            
        }
        console.warn("colorArr",typeArr)
        if(isInit ===0){
            ...
            matrixs =kernel_material_center.getFlushingVolumes(colorArr,typeArr,multiplier.realValue)
        }
    ...
    function setTipWarning(){
        for(let i=0;i<tableData.count;i++){
            ...
            for(let j=1;j<tableViewContainer.columnCount;j++){
                let value = tableData.get(i)[`num${j}`]
                console.warn(value,"setTipWarning2:",typeArr[i],typeArr[j-1],nozzleVolum,i,j)
                if(typeArr[i] !== typeArr[j-1])
                {
                    if(value<nozzleVolum*3){
                        tipColor2 = "red"
                        checkerror = true
                        break;
                    }else{
                        tipColor2 = Constants.textColor
                    }
                }
                if(j-1 === i) continue
...
//                    width: 100 //parseInt(tableContainer.width/tableData.count)
                ...
                itemDelegate:Rectangle{
                    ...
                    BasicDialogTextField {
                        ...
                        color:{
                            if(!text) return "red";
                            if(typeArr[styleData.row] !== typeArr[styleData.column-1]){
                                if(+styleData.value<nozzleVolum*3)
                                    return "red"
                            } 
                            if(+styleData.value>1200 &&enabled){
                                return "red"
                            }else if(+styleData.value<nozzleVolum&&enabled){
                                return "red"
                            }else{
                                return Constants.themeTextColor
                            }
                        } 
                        anchors.centerIn: parent
        ...
        ColumnLayout{
            ...
            StyledLabel {
                ...
                rightPadding: 10//
                color: tipColor2
                text: qsTr("Warm reminder: When it is flushing between different filaments,It is recommended that the flushing volume")+">"+ nozzleVolum*3
                font.pointSize: Constants.labelFontPointSize_9
            }
        }
