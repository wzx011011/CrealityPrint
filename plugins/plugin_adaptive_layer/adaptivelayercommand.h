...
{
  Q_OBJECT
  Q_PROPERTY(float speedVal READ speedVal WRITE setSpeedVal NOTIFY speedValChanged);
  Q_PROPERTY(int radiusVal READ radiusVal WRITE setRadiusVal NOTIFY radiusValChanged);
...
public:
  float speedVal() const 
  { 
      return m_speedVal; 
  }
  void setSpeedVal(const float& val) 
  { 
      m_speedVal = val; 
  ...
  void setRadiusVal(const int& val) { emit radiusValChanged();  m_radius = val; }
