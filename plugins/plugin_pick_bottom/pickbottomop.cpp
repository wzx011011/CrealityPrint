...
#include "qtuser3d/entity/selectentity.h"
...
{
	m_arrowEntity = new PureColorEntity();
	m_arrowEntity->setGeometry(PRIMITIVESHAPE("cylinder"));
	m_selectModel = new ModelN();
	m_selectFace = new SelectEntity();
}
...
{
	visShow(m_arrowEntity);
	visHide(m_arrowEntity);
...
void PickBottomOp::onLeftMouseButtonClick(QMouseEvent* event)
{
	...
	int primitiveID = -1;
	Pickable* pickable = checkPickable(event->pos(), &primitiveID);
	FacePickable* face = qobject_cast<FacePickable*>(pickable);
	if (face)
	{
		QVector3D z = QVector3D(0.0f, 0.0f, -1.0f);
		QMatrix4x4 qmatrix = m_selectModel->globalMatrix();
		QVector3D qNormal = QMatrix4x4(qmatrix.normalMatrix()) * QVector3D(face->gNormal().x, face->gNormal().y, face->gNormal().z);
		qNormal.normalize();
		QQuaternion q = quaternionFromVector3D2(qNormal, z, true);
		rotateModelOnPlane(m_selectModel, q);
	...
	//	if (model)
	//	{
	//		bool needFan = model->isFanZhuan();
	//		normal.normalize();
	//		QVector3D z = QVector3D(0.0f, 0.0f, -1.0f);
	//		QQuaternion q = quaternionFromVector3D2(normal, z, true);
...
void PickBottomOp::reGenerateFaces()
{
	...
	if (mesh)
	{
		...
		for (int n = 0; n < faceNum; n++)
		{
			...
			{
				if (false == mesht->faces[m].bUnique)
				{
					continue;
				}
	...
	{
		qtuser_3d::FacePickable* afacePickable = new qtuser_3d::FacePickable(mesh);
		tracePickable(afacePickable);
		afacePickable->getEntity()->setParent(m_selectModel->getModelEntity());
		m_pickFaces.push_back(afacePickable);
	}
}

void PickBottomOp::removeFaces()
{
	for (qtuser_3d::FacePickable* apickFace : m_pickFaces)
	{
		unTracePickable(apickFace);
	}
	qDeleteAll(m_pickFaces);
	m_pickFaces.clear();
}
