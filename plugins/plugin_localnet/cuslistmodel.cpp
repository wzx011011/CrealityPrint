...
bool CusListModel::checkOnePrint()
{
	...
	{
		auto item = pair.second;
		if (item->pcHasSelect() && (item->pcPrinterState() == 1||item->pcPrinterState() == 5))
		{
			hasPrinting = true;
			break;
		}
...
void CusListModel::batch_command(const QString& fileName, const QString& filePath, int plate, const QList<QVariantMap>& colorMap)
{
	...
	foreach(auto pair, m_ListPrinterItem)
	{
		...
		if (item->pcHasSelect())
		{
			...
			printer.type = RemotePrinerType(item->printerType());
			item->setTransErrorMessage("Send pending");
			item->setGCodeTransProgress(-3);
			onDataChanged(printer.macAddress, QVector<int>() << ListModelItem::E_TransErrorMessage);
...
void CusListModel::slotGetInfoSuccess(const RemotePrinter& stateInfo)
{
	...
	item->setMaxBoxTemp(stateInfo.maxBoxTemp);
	...
	if(stateInfo.remoteFileUpdateTime!=0)
	{
	...
	if (stateInfo.type == RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408)
	{
		item->setMachineWidth(get_machine_width(settings.data()));
		item->setMachineHeight(get_machine_height(settings.data()));
		item->setMachineDepth(get_machine_depth(settings.data()));
	}
...
void CusListModel::UploadProgressCallBack(const std::string& strMac, const std::string& fileName, const float& progress)
{
	...
	if (item)
	{
		...
		if (success)
		{
			...
			if(item->filelistModel()!=nullptr)
			{
				bool isGcodeExist = false;
				FileListModel* filelistModel = reinterpret_cast<FileListModel*>(item->filelistModel());
				item->setTransErrorMessage("The machine is processing file");
				//最多等待10s
				while(m_sendOnly && !isGcodeExist && timer.elapsed() < 15000 && item->pcPrinterStatus() == 1)
				{
					item->setGCodeTransProgress(-2);
					isGcodeExist = filelistModel->isGcodeFileExist(QString::fromStdString(strMac), QString::fromStdString(fileName));
...
int ListModelItem::maxBoxTemp() const
{
	return m_maxBoxTemp;
}

void ListModelItem::setMaxBoxTemp(const int temp)
{
	if (m_maxBoxTemp == temp)
		return;

	m_maxBoxTemp = temp;
	emit maxBoxTempChanged();
}
...
void ListModelItem::cancelUploadFile()
{
	getRemotePrinterManager()->cancelUploadFile(m_pcIpAddr);
}
