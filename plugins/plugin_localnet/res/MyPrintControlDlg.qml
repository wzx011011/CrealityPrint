...
BasicDialog{
    ...
    property var videoPlaying: false
    property var ignoreCount : 10
    ...
    property var curMac : ""
    property string curDeviceName: ""
    property var curPrinterName: ""
    property var curPrintState: 0 //打印状态 0：空闲 1：打印中 2：打印完成 3：打印失败 4：停止打印 5：打印暂停
    ...
    Item{//Window组件没有states这个属性，故添加Item
        states: [
            State {
                name: "Idle"
                when: curPrintState === 0 || curPrintState === 2 || curPrintState === 3 || curPrintState === 4
                PropertyChanges { target: idPrintState; text: qsTr("Idle") }
                PropertyChanges { target: idGcodeCombox; enabled: true }
                PropertyChanges { target: idStartPrintBtn; visible: true }
                PropertyChanges { target: idPausePrintBtn; visible: false }
                PropertyChanges { target: idContinuePrintBtn; visible: false }
                PropertyChanges { target: idStopPrintBtn; visible: false }
                PropertyChanges { target: idControllerBtn; visible: true }
            },
            State {
                name: "Printing"
                when: curPrintState === 1
                PropertyChanges { target: idPrintState; text: qsTr("Printing") }
                PropertyChanges { target: idGcodeCombox; enabled: false }
                PropertyChanges { target: idStartPrintBtn; visible: false }
                PropertyChanges { target: idPausePrintBtn; visible: true }
                PropertyChanges { target: idContinuePrintBtn; visible: false }
                PropertyChanges { target: idStopPrintBtn; visible: true }
                PropertyChanges { target: idControllerBtn; visible: false }
            },
            State {
                name: "printFinished"
            },
            ...
            State {
                name: "PrintPause"
                when: curPrintState === 5
                PropertyChanges { target: idPrintState; text: qsTr("Printing pause") }
                PropertyChanges { target: idGcodeCombox; enabled: false }
                PropertyChanges { target: idStartPrintBtn; visible: false }
                PropertyChanges { target: idPausePrintBtn; visible: false }
                PropertyChanges { target: idContinuePrintBtn; visible: true }
                PropertyChanges { target: idStopPrintBtn; visible: true }
                PropertyChanges { target: idControllerBtn; visible: false }
            }
        ]
                    ...
                    Column{
                            ...
                            Row{
                                ...
                                BasicCombobox{
                                    ...
                                    onComboBoxIndexChanged: {
                                        ...
                                        {
                                            idDeviceType.text = "  " + idGcodeListModel.get(currentIndex).modelData
                        ...
                        Row{
                            ...
                            PrintControlSpinBox {
                                ...
                                realTo:300
                                realValue: 100
                                //enabled : (curPrintState ==0 || curPrintState ==2 || curPrintState ==4)
                                onValueEdited:{
                                    //set temp
                                    if(curMac != "")
                                    {
                        ...
                        Column{
                            ...
                            Row{
                                ...
                                CusSkinButton_Image{
                                    ...
                                    onClicked:
                                    {
                                        if(curMac != "")
                                        {
                                            ...
                                            {
                                                moveCurAxisY = (parseFloat(moveCurAxisY) + parseFloat(0.1)).toFixed(1)
                                            }
                                            if(idMove1mm.checked == true)
                                            {
                                                moveCurAxisY = (parseFloat(moveCurAxisY) + parseFloat(1)).toFixed(1)
                                            }
                                            if(idMove10mm.checked == true)
                                            {
                                                moveCurAxisY = (parseFloat(moveCurAxisY) + parseFloat(10)).toFixed(1)
                                            }
                                ...
                                CusSkinButton_Image{
                                    ...
                                    enabled: {
                                        if(!isReverseZAxisOperation(/*printerNameMap[deviceId]*/curPrinterName))
                                        {
                                            return true
                                        }
                                        else
                                    ...
                                    onClicked:
                                    {
                                        if(curMac != "")
                                        {
                                            ...
                                            if(idMove01mm.checked == true)
                                            {
                                                moveCurAxisZ = isReverseZAxisOperation(/*IotData.iotPrinterNameMap[deviceId]*/curPrinterName) ? ((parseFloat(moveCurAxisZ) - parseFloat(0.1)).toFixed(1)) : ((parseFloat(moveCurAxisZ) + parseFloat(0.1)).toFixed(1))
                                            }
                                            if(idMove1mm.checked == true)
                                            {
                                                moveCurAxisZ = isReverseZAxisOperation(/*IotData.iotPrinterNameMap[deviceId]*/curPrinterName) ? ((parseFloat(moveCurAxisZ) - parseFloat(1)).toFixed(1)) : ((parseFloat(moveCurAxisZ) + parseFloat(1)).toFixed(1))
                                            }
                                            if(idMove10mm.checked == true)
                                            {
                                                moveCurAxisZ = isReverseZAxisOperation(/*IotData.iotPrinterNameMap[deviceId]*/curPrinterName) ? ((parseFloat(moveCurAxisZ) - parseFloat(10)).toFixed(1)) : ((parseFloat(moveCurAxisZ) + parseFloat(10)).toFixed(1))
                                            }
                                ...
                                CusSkinButton_Image{
                                    ...
                                    onClicked:
                                    {
                                        if(curMac != "")
                                        {
                                            ...
                                            if(idMove01mm.checked == true)
                                            {
                                                moveCurAxisX = (parseFloat(moveCurAxisX) - parseFloat(0.1)).toFixed(1)
                                            }
                                            if(idMove1mm.checked == true)
                                            {
                                                moveCurAxisX = (parseFloat(moveCurAxisX) - parseFloat(1)).toFixed(1)
                                            }
                                            if(idMove10mm.checked == true)
                                            {
                                                moveCurAxisX = (parseFloat(moveCurAxisX) - parseFloat(10)).toFixed(1)
                                            }
                                ...
                                CusSkinButton_Image{
                                    ...
                                    onClicked:
                                    {
                                        if(curMac != "")
                                        {
                                            ...
                                            moveCurAxisX = parseFloat(0).toFixed(1)
                                            moveCurAxisY = parseFloat(0).toFixed(1)
                                ...
                                CusSkinButton_Image{
                                    ...
                                    onClicked:
                                    {
                                        if(curMac != "")
                                        {
                                            ...
                                            moveCurAxisZ = parseFloat(0).toFixed(1)
    ...
    onDialogClosed:
    {
        ...
        if(videoPlaying)
        {
            idvideo.stopVideo()
            videoPlaying = false
            idvideo.hasVidew = false
            idvideo.visible = false
        }
    }

    Connections{
        target:cwModel
        onSigPrinterStateChanged:{
            if(idPrintControlInfoDlg.visible == false)
            {
                return ;
            }            
            if(curMac != dn)
                return
            curDeviceName = ps["deviceName"]
            curPrinterName = ps["printerName"]
            curPrintState = ps["printState"]
            curFanSwitchState = ps["fanSwitchState"]
            curMyPosition = ps["curPosition"]
            curPrintJobTime = ps["printJobTime"]
            curNozzleTemp = ps["nozzleTemp"]
            if(isNaN(ps["nozzleTempTarget"]))
            {
                curNozzleTempTarget = 0
            }
            else
            {
                curNozzleTempTarget = ps["nozzleTempTarget"]
            }
            curBedTemp = ps["bedTemp"]
            if(isNaN(ps["begTempTarget"]))
            {
                curBegTempTarget = 0
            }
            else
            {
                curBegTempTarget = ps["begTempTarget"]
            }

            curPrintSpeed = ps["printSpeed"]
            curPrintProgress = ps["printProgress"]
            printVideo = ps["video"]

            idvideo.visible = true
            if(printVideo == 1 && printerIP != "" && videoPlaying == false && idPrintControlInfoDlg.visible == true)
            {
                urlStr = "rtsp://" + printerIP + "/ch0_0"
                idvideo.loadVideoUrl(urlStr)
                idvideo.hasVidew = true
                videoPlaying = true
            }
            printerIP = ps["ipAddress"]
            curPrintFile = ps["printFileName"]
            if(curPrintFile != "")
            {
                var index = idGcodeCombox.find(curPrintFile)
                console.log("set cur file name index=" + index)
                if(index != -1)
                {
                    idGcodeCombox.currentIndex = index
                }
            }
            updateQMLData()
        }
    ...
    function initCurMac(mac, curState)
    {
        curMac = mac
        curPrintState = curState
    }
    ...
    function clearQMLData()
    {
        ...
        idGcodeListModel.clear()
        //idGcodeCombox.displayText = ""
        idDeviceType.text = qsTr("Please select a print file")
        idNozzleTempTargetLabel.realValue = 0
        idNozzleTempText.text =" ℃"
        idBedtempTargetLabel.realValue = 0
        idBedTempText.text = " ℃"
    ...
    function updateQMLData()
    {
        ...
        if(str)
        {
            ...
            if(arr[0].split(":")[1] != undefined)
            {
                ...
                {
                    moveCurAxisX = parseFloat(arr[0].split(":")[1]).toFixed(1)
                }
                moveCurRealAxisX = parseFloat(arr[0].split(":")[1]).toFixed(1)
            }
            ...
            if(arr[1].split(":")[1] != undefined)
            {
                ...
                {
                    moveCurAxisY = parseFloat(arr[1].split(":")[1]).toFixed(1)
                }
                moveCurRealAxisY = parseFloat(arr[1].split(":")[1]).toFixed(1)
            }
            ...
            if(arr[2].split(":")[1] != undefined)
            {
                ...
                {
                    moveCurAxisZ = parseFloat(arr[2].split(":")[1]).toFixed(1)
                }

                moveCurRealAxisZ = parseFloat(arr[2].split(":")[1]).toFixed(1)
            }
        }
        ...
        if(nozzleTargetTempIsChanged == false)
        {
            ...
            if(nozzleTargetTempCount == ignoreCount)
            {
        ...
        if(hotBedTargetTempIsChanged == false)
        {
            ...
            if(hotBedTargetTempCount == ignoreCount)
            {
        ...
        if(printSpeedTargetTempIsChanged == false)
        {
            ...
            if(printSpeedTargetTempCount == ignoreCount)
            {
