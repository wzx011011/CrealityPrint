...
QVariant GcodeListModel::data(const QModelIndex& index, int role) const
{
	...
	switch (role)
	{
		...
		case GcodeModelItem::E_GCodeFileNames:
			...
			break;
		case GcodeModelItem::E_GCodeTime:
			dateTime = QDateTime::fromString(modelItem->gCodeFileTime(), "yyyy-MM-dd hh:mm:ss");
			if(dateTime.isValid())
				secondsSinceEpoch = dateTime.toSecsSinceEpoch();
			else
				secondsSinceEpoch = QDateTime::currentDateTime().toSecsSinceEpoch();
			value = QString::number(secondsSinceEpoch);
			break;
		default:
			break;
	}
...
QHash<int, QByteArray> GcodeListModel::roleNames() const
{
	...
	roles[GcodeModelItem::E_GCodeTime]		 = "gcodeTime";
		...
		case RemotePrinerType::REMOTE_PRINTER_TYPE_KLIPPER4408:
		{
			...
			foreach(QString fullInfo, splitList)
			{
				if (m_webInfoRegExp.exactMatch(fullInfo))
				{
					...
					QString thumbnailImage = ":80/downloads" + QUrl::toPercentEncoding(m_webInfoRegExp.cap(6));
