...
void VideoDecoder::startPlay(const QString& strUrl)
{
	...
	int target_fps = 12;
	int64_t frame_duration = (int64_t)((1.0 / target_fps)*1000*1000);
	...
	while (!isStop)
	{
		do {
			...
			if (pkt.stream_index == video_st_index)
			{
				...
				auto duration = std::chrono::duration_cast<std::chrono::microseconds>(current_time - last_frame_time);
				if (duration.count() < frame_duration) {
						// 如果时间未到期望的下一帧时间间隔，进行延时等待
						int64_t sleep_time_us = frame_duration - duration.count();
						std::this_thread::sleep_for(std::chrono::milliseconds(sleep_time_us/1000));
				}
				
				
				// 在这里可以进行实际的帧处理，比如解码显示等（此处省略解码显示相关代码）
